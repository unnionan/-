...
public class GetUserUpdatePwd extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(f.update_userPwd(Id,  loginpwd))
		{
			response.getWriter().print("<script language = 'javascript'>alert('修改用户密码成功')</script>");
			 response.setHeader("refresh","1;AdminBottom.jsp");
		}
		else
		{
			response.getWriter().print("<script language = 'javascript'>alert('修改用户密码失败')</script>");
			 response.setHeader("refresh","1;AdminBottom.jsp");
		}
		f.CloseConnection();
	}
